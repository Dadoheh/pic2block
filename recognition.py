    ...
    def _clear_up_similar_inputs_from_inputs(self) -> None:
        ...
        for element in self.inputs:
            ...
            if not abs((self.inputs.index(element)) - len(self.inputs)) < 0:
                for sub_element in range(self.inputs.index(element)+1, len(self.inputs)):
                    x_sub_cord = int((re.search('c\.x:([0-9]*)', self.inputs[sub_element])).group(1))
                    if abs(x_cord - x_sub_cord) < 5:
                        logger.debug(f'x_sub_cord: {x_sub_cord}')
                        list_of_repetitive_items.append(self.inputs[sub_element])  # pes. x^2 complexity - to refactor
